...
const STEP_A_SYSTEM =
  'You are the data generation engine for REGULENS, a Nigerian regulatory intelligence platform. You produce realistic, internally-consistent workspace datasets for a national regulator scenario. Respond with ONLY valid JSON. No markdown, no prose outside the JSON.';

async function stepA() {
  const user = `Generate TWO lists for an African (Nigeria-focused) regulatory workspace:

1) businesses: exactly 12 companies across industries that face Nigerian regulation (food & beverage, pharmaceuticals, banking/fintech, energy/power distribution, telecoms, agricultural exports, construction, manufacturing, healthcare, logistics, insurance, consumer retail). For each: id ("BSN-001".."BSN-012"), name, industry, location (a Nigerian city/zone), impact ("High"|"Medium"|"Low"), risk ("High"|"Medium"|"Low"), status ("Affected"|"At Risk"|"Monitored"). Give realistic fictional company names.

2) regulations: exactly 12 Nigerian regulatory instruments. For each: id ("REG-1001".."REG-1012"), title (a realistic regulation/order/directive name), authority (realistic Nigerian regulator), jurisdiction ("Federal"|"State"), effectiveDate ("YYYY-MM-DD" in 2025-2026), impact ("High"|"Medium"|"Low"), status ("Active"|"Pending"|"Under Review"), sections (integer 15-140), description (1-2 sentences on what it governs).
