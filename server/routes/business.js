  ...
  if (head === "document-review") {
    ...
    if (!text && body.file && typeof body.file === "object") {
      try {
        ...
        const reply = await bizDocumentUnreadable(body.file, err.message || "the file could not be read");
        return ok(res, { reply });
      }
    }
    if (!text) {
      if (body.file && typeof body.file === "object") {
        const reply = await bizDocumentUnreadable(body.file, "no readable text was found inside the file");
        return ok(res, { reply });
      }
...
async function bizDocumentUnreadable(file, reason) {
  ...
  return groqWithFallback(
    ...
    {
      system:
        "You are a helpful compliance assistant for REGULENS, an Indian regulatory intelligence platform for businesses. Reply concisely, in very simple language, with practical next steps.",
      maxTokens: 700,
      fallback: () =>
